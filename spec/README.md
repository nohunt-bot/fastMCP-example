...
目前宣告 **L2**。這個宣告在 2026-08-07 的稽核中一度是**假的**，修正的
過程本身值得記錄，因為它示範了「可執行的規範」比「寫得好的規範」重要在
哪裡。
...
| 問題 | 條文 | 現況 |
|---|---|---|
| 驗證器容許豁免不可豁免的安全規則 | RFC-057 | 已修正 |
| 錯誤未帶結構化欄位，全是字串 | RFC-100 | 已實作 |
| `user_message` 帶容器絕對路徑 | RFC-101 | 已修正 |
| Schema 缺 `maxLength` / `maxItems` | RFC-110/113 | 已補上 |
| 重名 skill 的勝出者取決於檔案系統順序 | RFC-033 | 已改為路徑排序 |

最值得記下的是第一項。RFC-057 明文規定 `error` 等級的安全規則不可豁免，
但 `validate.py` 的 `flag()` 對所有規則一視同仁地接受 `# spec:allow`。
結果是：一支 `requests.get()` 完全沒有 timeout 的 script——正是本專案存在
理由的那種當機——加上一行註解就能讓驗證器印出「通過 L2」。

**規範寫對了，執行規範的程式沒有。** 而由於通過的是驗證器、不是條文，
沒有人會發現。這也是為什麼本規範堅持要有可執行形式：一份沒有驗證器的
規範，只能約束願意讀它的人。

未達 L3 的原因見專案 README：條文說效能數字不得為推算，2,000 / 5,000 個
skill 的冷啟動數字目前是推算的，那就不能宣稱達標。這是規範能約束提出它
的專案的實例。
