...
**RFC-002** 一般請求 MUST NOT 觸發完整重新掃描。

明確觸發的重載（`reload_skills`）不受此限——它是刻意的操作，不是每次請求
都要付的隱含成本。

**RFC-002a** 背景週期性掃描 MUST 為可關閉，且當 Skill 隨映像檔一起發佈時
SHOULD 預設關閉。

**理由**：映像檔內的 Skill 在 Pod 生命週期內不會變動，掃描只會消耗 CPU 去
確認「什麼都沒變」。實測 2,000 個 Skill 在 0.1 core 上每 5 秒消耗 14% 的
CPU 配額。
