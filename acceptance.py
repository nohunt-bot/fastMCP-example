...
GROUPS: list[Group] = [
    ...
    Group(
        ...
        items=[
            ...
            Item("SKILL_STATE_DIR 跨呼叫保存", "test_state_dir_is_exposed_and_persists_between_calls",
                 "key 需要活得比模型的 context 久。", "檢查 --state-dir 與寫入權限。"),
            Item("各 skill 的 state 互相隔離", "test_state_dirs_are_isolated_per_skill",
                 "避免 skill 之間互相污染。", "檢查 runner.py 的 state_root / skill 組合。"),
            Item("送出的 key 會寫進 ledger", "test_submitted_keys_survive_in_the_ledger",
                 "context 滾掉之後唯一的救援途徑。", "檢查 job.py 的 record() 與 SKILL_STATE_DIR。"),
            Item("ledger 併發寫入不遺失", "test_ledger_is_append_only_under_concurrent_submits",
                 "同時送出多個任務時不能互相覆蓋。", "確認是 append 模式，不是讀取後整份覆寫。"),
        ],
    ),
    Group(
        title="MCP 介面",
