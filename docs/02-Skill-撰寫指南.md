...
檢索用 **CJK bigram + BM25**，中英文都可以。實測 13 個真實中文查詢的第一名
命中率為 85%（改用 BM25 之前是 8%）。

欄位權重：`name` × 3、`tags` × 2、`description` × 1。

### description 要包含使用者實際會用的詞

n-gram 比對的是**字面**，不是語意。這是唯一需要你配合的地方：

| description 寫法 | 使用者問「今天賣了多少」 |
|---|---|
| 「產生每日營運報表，包含**銷售額**與訂單數」 | ✗ 找不到——「賣」和「銷售」字面不同 |
| 「產生每日營運報表，包含**銷售額**（賣了多少）與訂單數」 | ✓ |

實測中唯一失敗的案例就是這個。解法是在 description 裡帶上口語說法：

```yaml
# 只有正式術語
description: 產生每日營運報表，包含銷售額、訂單數與退貨率的統計摘要。

# 加上使用者會用的詞
description: 產生每日營運報表，包含銷售額（今天賣了多少）、訂單數與退貨率。當使用者問到業績、營收或銷售狀況時使用。
```

### 300 個 description 必須彼此分得開

單看一個 description 永遠覺得合格，問題在**擺在一起**時才出現——而模型
看到的正是擺在一起的樣子。

```bash
uv run python -m spec.validate skills/ --level=L2
```

會標出相似度過高的組合：

```
[warn] LINT-040  description 與 'order-query' 相似度 57%，模型可能選錯（門檻 35%）
```
...
| 寫法 | 相似度 |
|---|---|
| 「查詢訂單狀態。提供訂單狀態的查詢功能。」 | 0.43–0.57 ✗ |
| 「依訂單編號查出貨進度。當使用者問『我的東西寄到哪』時使用。」 | 0.11–0.14 ✓ |

差別在後者寫了**何時使用**與**使用者實際會說的話**，前者只描述功能。

```yaml
# 分不開 —— 五個都長這樣
description: 查詢訂單狀態。提供訂單狀態的查詢功能。

# 分得開
description: 依訂單編號查出貨進度與物流單號。當使用者問「我的東西寄到哪」或提供訂單編號時使用。
```

### 同義詞直接寫進去

沒有同義詞擴展機制（那需要詞典，會拖慢啟動）。有同義詞就並列：

```yaml
description: 追蹤出貨物流狀態與預計到貨時間（貨到哪了、什麼時候送到）。
```

### tags 決定領域總覽

skill 多到目錄放不進 context 時，`list_skills` 會改回傳**領域總覽**：

```json
{"view": "overview",
 "facets": {"order": 15, "inventory": 15, "shipping": 15, ...},
 "skills": [每個領域一個代表],
 "hint": "用 tags=['<領域>'] 取得完整清單"}
```

實測 315 個 skill 時，總覽只花 875 tokens（30k 的 3%）就涵蓋全部 21 個領域。

**沒有 tags 的 skill 以名稱第一段分組**（`order-lookup` → `order`），能用但
不精確。所以：

- **一定要寫 tags**
- **命名用領域前綴**：`order-lookup`、`order-refund`，不要 `lookup-order`
